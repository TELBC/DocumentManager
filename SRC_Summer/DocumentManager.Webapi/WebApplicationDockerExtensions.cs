...
public static class WebApplicationDockerExtensions
{
        ...
        string connectionString, bool deleteAfterShutdown = true)
    {
        ...
        var containerParameters = new CreateContainerParameters
        {
            ...
            Name = containerName,
            Env = new[] { $"POSTGRES_USER={username}", $"POSTGRES_PASSWORD={password}" },
            HostConfig = new HostConfig
            {
                PortBindings = new Dictionary<string, IList<PortBinding>>
                {
                    { "5432/tcp", new PortBinding[] { new() { HostPort = $"{port}/tcp" } } }
                }
            }
        };
        // We wait for a log message "database system is ready to accept connections".
        await StartContainer(
            app, containerName, containerParameters,
            "database system is ready to accept connections", deleteAfterShutdown: deleteAfterShutdown);
    }
        ...
        bool deleteAfterShutdown = true)
    {
        ...
        {
            await DeleteContainer(containerName, true);
            app.Lifetime.ApplicationStopped.Register(() =>
            {
                // Wait synchronously because this is an Action<T> not a Func<Task>
                DeleteContainer(containerName, true).Wait(5000);
            });
        }

        var containers = await client.Containers.ListContainersAsync(new ContainersListParameters { All = true });
        var containerId = containers.FirstOrDefault(c => c.Names.Any(n => n == $"/{containerName}"))?.ID;
        if (containerId is null) containerId = (await client.Containers.CreateContainerAsync(containerParameters)).ID;
        ...
        if (waitCommand is not null)
        {
            ...
            if (!result)
                throw new Exception(
                    $"Container {containerName} could not start. {string.Join(" ", waitCommand)} returns not {waitForMessage} or exit code 0.");
        }
        else
        {
    ...
    private static async Task<bool> WaitForOutputCommand(DockerClient client, string containerId, string[] waitCommand,
    ...
    /// </summary>
    private static async Task<bool> WaitForOutputLog(DockerClient client, string containerId, string waitForMessage,
        int timeout = 30)
    {
